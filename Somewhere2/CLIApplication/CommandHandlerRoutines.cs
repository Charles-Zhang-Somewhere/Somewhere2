...
namespace Somewhere2.CLIApplication
{
    internal partial class CommandHandler
    {
        ...
        {
            string text = Helpers.ReadTextResource(Assembly.GetExecutingAssembly(), "Somewhere2.Documentation.Welcome.txt");
        ...
        private void PrintIntroText(string databasePath)
        {
            ...
            string text = Helpers.ReadTextResource(Assembly.GetExecutingAssembly(), "Somewhere2.Documentation.NewDatabase.txt");
            ColorfulPrintLine(text);
        }
        private void RunGUI()
        {
            ...
            new MainApplication(RuntimeContext).Run();
        }
                ...
                TextEditor textEditor = new TextEditor(new YamlDotNet.Serialization.Serializer().Serialize(RuntimeData.Configuration),
                    text =>
                    {
                        RuntimeData.Configuration = new YamlDotNet.Serialization.Deserializer()
                            ...
                            FileService.SaveConfig(RuntimeData.Configuration);
                    });
                textEditor.Show();
            });
        }
        ...
        private void ShowStatsWindow()
            => new StatsWindow(RuntimeContext).Run();
